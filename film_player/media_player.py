...
class Playlist:
    ...
    def add_film_favorites(self):
        ...
        for all_films in data:
            if '2021' in all_films:
                films_2021 = all_films
                #print(films_2021)
                if "Безкоштовно" in films_2021:
                    films_2021_free = films_2021[films_2021.find('https'): films_2021.find('html') + 4]
                    url = films_2021_free
                    py_headers = ({'User-Agent': 'Safari/537.36', 'Accept-Language': 'en-US, en;q=0.5'})
                    py_wpage = requests.get(url, headers=py_headers)
                    py_soup = BeautifulSoup(py_wpage.content, "html.parser")
...
class Player:
    ...
    def pause(self):
        self.time = f'{str(self.time)} хвилині https://drive.google.com/file/d/1h_2eNeBem3GKLYE6BLe9dthdjlAx7kVw/view?usp=sharing'
...
player1 = Player('Секс заради виживання','https://megogo.net/ua/view/3416081-seks-zaradi-vizhivannya.html ')
print(player1.play())
print(player1.pause())
